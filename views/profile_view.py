...
def profile_content(page: ft.Page, user_id: str = None, on_view_profile=None):
    ...
    header_card = ft.Container(
            ...
            controls=[
                    ...
                    controls=[
                        ft.Row(
                            controls=[
                                ...
                                *(
                                    [ft.Container(width=8, height=8, bgcolor=ACCENT_SUCCESS,
                                                  border_radius=50)] if user.is_online else []
                                ),
                            ],
                            spacing=8,
                ...
                *(
                    [] if is_own else [
                        ft.Container(
                            content=ft.Text(
                                "Conectado" if is_connected else "Conectar",
                                color=TEXT_PRIMARY, size=13, weight=ft.FontWeight.W_600,
                            ),
                            bgcolor=ACCENT_SUCCESS if is_connected else ACCENT_PRIMARY,
                            border_radius=10,
                            padding=ft.padding.symmetric(horizontal=20, vertical=10),
                            ink=True,
                            on_click=handle_connect if not is_connected else None,
                        ),
                    ]
                ),
            ],
        ...
        bgcolor=BG_CARD, border_radius=16, border=ft.border.all(1, BORDER_COLOR),
        padding=24,
    )
    ...
    skills_section = ft.Container(
        content=ft.Column(
            controls=[
                ...
                ft.Row(
                    controls=[skill_tag(s, size="medium") for s in user.skills],
        ...
        proj_items.append(ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.Container(
                        content=ft.Text(status_map.get(p.status, p.status),
                                        color=status_colors.get(p.status, TEXT_SECONDARY), size=11),
    ...
    projects_section = ft.Container(
        content=ft.Column(
            controls=[
                ft.Text("Proyectos", color=TEXT_PRIMARY, size=16, weight=ft.FontWeight.W_600),
    ...
    for other_user, count, shared in similar[:5]:
        sim_items.append(ft.Container(
            content=ft.Row(
                controls=[
                    ft.CircleAvatar(
                        content=ft.Text(other_user.initials, size=11, weight=ft.FontWeight.BOLD,
                                        color=TEXT_PRIMARY),
    ...
    similar_section = ft.Container(
        content=ft.Column(
            controls=[
                ft.Text("Habilidades Similares", color=TEXT_PRIMARY, size=16, weight=ft.FontWeight.W_600),
            ] + sim_items,
        ),
        bgcolor=BG_CARD, border_radius=14, border=ft.border.all(1, BORDER_COLOR), padding=20,
    )
    ...
    back_btn = ft.Container(
        ...
        on_click=lambda e: page.go("/app/dashboard"),
        ink=True,
        border_radius=8,
